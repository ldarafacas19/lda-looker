  ...
  derived_table: {
    sql: WITH filtered_sales AS (
          SELECT
              ...
              r.loyalty_index,
              r.total_orders,
              COALESCE(r.group, CASE WHEN RIGHT(CAST(s.user_id AS STRING), 1) = '9' THEN 'group_control' ELSE 'group_test' END) AS user_group
          FROM
      ...
      daily_metrics AS (
          ...
          GROUP BY organization_id, organization_name, year, month, day, order_date, user_group, total_orders
      ),
      ...
      aggregated_data AS (
          ...
          GROUP BY
              co.organization_name, co.order_date, co.year, co.month, co.day, fd.user_group, fd.usuarios_acumulados, co.total_orders
      ),
      ...
      grouped_final AS (
          ...
          FROM
              aggregated_final af
          GROUP BY
              af.organization_name, af.order_date, af.year, af.month, af.day, af.total_orders
      )
